...
export function useVideoConsultation({
  ...
  const ensureVideoElementsExist = () => {
    ...
    if (!remoteVideoElementRef.current) {
      ...
      console.log('[patient-webrtc] Pre-created remote video element');
    }
    ...
    if (!localVideoElementRef.current) {
      ...
      console.log('[patient-webrtc] Pre-created local video element');
    }
    ...
    if (!remoteAudioElementRef.current) {
      ...
      console.log('[patient-webrtc] Pre-created remote audio element');
    }
  };
    ...
    try {
      ...
      if (playPromise && typeof playPromise.catch === 'function') {
        playPromise.catch((playError) => {
          console.warn(`[patient-webrtc] ${label} autoplay blocked, waiting for user gesture:`, playError);
          ...
          const retryPlayback = () => {
            element.play()
              .then(() => {
                ...
                console.log(`[patient-webrtc] ${label} playback resumed after user gesture`);
              })
              .catch((retryError) => {
                console.warn(`[patient-webrtc] ${label} playback retry failed:`, retryError);
              });
          };
      ...
      console.warn(`[patient-webrtc] ${label} playback error:`, playError);
    }
  };
  ...
  const mountStreams = (container: HTMLDivElement) => {
    ...
    if (remoteStreamRef.current) {
      ...
      console.log('[patient-webrtc] Mounted with existing remote stream');
      attemptMediaPlayback(remoteVideo, 'Remote video');
      attemptMediaPlayback(remoteAudio, 'Remote audio');
    } else {
      console.log('[patient-webrtc] Mounted with null remote stream - will bind on ontrack');
    }

    if (localStreamRef.current) {
      localVideo.srcObject = localStreamRef.current;
      console.log('[patient-webrtc] Local stream mounted');
    }
    ...
    console.log('[patient-webrtc] Video elements mounted to container');
  };
  ...
  const initializePeerConnection = () => {
    ...
    pc.ontrack = (event) => {
      console.log('[patient-webrtc] ontrack event fired:', {
      ...
      if (!remoteStreamRef.current) {
        ...
        console.log('[patient-webrtc] Created new MediaStream for remote');
      }

      event.streams[0].getTracks().forEach((track) => {
        remoteStreamRef.current?.addTrack(track);
        console.log('[patient-webrtc] Added track to remote stream:', track.kind);
      });
      ...
      if (remoteVideoElementRef.current) {
        if (remoteStreamRef.current) {
          ...
          console.log('[patient-webrtc] Bound remote stream to video element, stream tracks:', remoteStreamRef.current.getTracks().length);
          attemptMediaPlayback(remoteVideoElementRef.current, 'Remote video');
        }
      } else {
        console.warn('[patient-webrtc] remoteVideoElementRef is null!');
      }
      ...
      setParticipants((prev) => {
        if (prev.find((p) => p.kind === 'remote')) return prev;
        return [...prev, { kind: 'remote' }];
      });
    };

    pc.onconnectionstatechange = () => {
      setConnectionState(pc.connectionState);
      console.log('[patient-webrtc] connectionState:', pc.connectionState);
    ...
    pc.oniceconnectionstatechange = () => {
      ...
      console.log('[patient-webrtc] iceConnectionState:', pc.iceConnectionState);
    };

    pc.onsignalingstatechange = () => {
      setSignalingState(pc.signalingState);
      console.log('[patient-webrtc] signalingState:', pc.signalingState);
    };
  ...
  const getLocalMediaStream = async () => {
    ...
    try {
      ...
      console.warn('[patient-webrtc] Camera unavailable, falling back to audio-only:', mediaError);
  ...
  const initializeCall = async () => {
    try {
      ...
      socket.on('connect', () => {
        ...
        console.log('[patient-webrtc] socket connected:', socket.id);
      });

      socket.on('disconnect', (reason) => {
        setSocketConnected(false);
        console.log('[patient-webrtc] socket disconnected:', reason);
      });
      ...
      socket.on('webrtc:room-joined', async (event) => {
        console.log('[patient-webrtc] room joined:', event?.roomName, 'role:', signalConfig?.role, 'peers:', event?.peers?.length || 0);
        const peers = Array.isArray(event?.peers) ? event.peers : [];
        if (signalConfig?.role === 'doctor' && peers.length > 0) {
      ...
      socket.on('webrtc:peer-joined', async (event) => {
        if (signalConfig?.role !== 'doctor') return;
      ...
      socket.on('webrtc:offer', async (event) => {
        ...
        if (signalConfig?.role !== 'patient') return;

        console.log('[patient-webrtc] received offer from:', event.senderSocketId);

        remoteSocketIdRef.current = event.senderSocketId || null;
        ...
        socket.emit('webrtc:answer', {
          targetSocketId: event.senderSocketId,
          sdp: answer,
        });
      });
      ...
      socket.on('webrtc:answer', async (event) => {
        ...
        if (pc.signalingState !== 'have-local-offer') {
          console.log('[patient-webrtc] ignoring answer due to signalingState:', pc.signalingState);
          return;
        }

        console.log('[patient-webrtc] received answer from:', event.senderSocketId);
